...
void InitRoad ()
{
	...
	shm.sems[NUMPOS+1] = Seminit(0); // west starts as open
	shm.sems[NUMPOS+2] = Seminit(0); // east starts as open
}
	...
	int mph;			// speed of car
{
	...
	// WAIT WEST OR EAST SIDE IF IT IS NOT THE VERY FIRST ONE
	
	if (shm.eastcars || shm.westcars || shm.waitingeast || shm. waitingwest){
		if (from == WEST){
				shm.waitingwest += 1; 
				Wait(shm.sems[NUMPOS+1]);
		}
		if (from == EAST){
				shm.waitingeast += 1;
				Wait(shm.sems[NUMPOS+2]);
		}
	}
	
	// THIS GETS INCREMENTED VERY SOON AFTER IT ENTERS THE ROAD
	
	else{
		if (from == WEST) shm.waitingwest++;
		if (from == EAST) shm.waitingeast++;
	}
	...
	//PROCEED ROAD, OPENING SPOTS BEHIND IT AS BLOCKING THOSE IN FRONT
	...
	for (i = 1; i < NUMPOS; i++) {
		...
		if (from == WEST){
			if (!shm.waitingeast && !shm.eastcars){
				if (shm.waitingwest > 0){
					Signal(shm.sems[NUMPOS+1]);
				}
			}
		}
		else{
			if (!shm.waitingwest && !shm.westcars){
				if (shm.waitingeast > 0){
					Signal(shm.sems[NUMPOS+2]);
				}
			}
		}
	} 
	
	//PROCEEDING ROAD TO END
	
	
	Delay (3600/mph);
	ProceedRoad ();
	PrintRoad ();
	if (from == WEST) Signal (shm.sems[NUMPOS]);
	else{
		Signal (shm.sems[1]);
	}
	...
	if (from == WEST){
		shm.westcars --;
		if ( !shm.westcars && shm.waitingeast ){
			Signal(shm.sems[NUMPOS+2]);
		}
		else if(shm.waitingwest && !shm.waitingeast){
			Signal(shm.sems[NUMPOS+1]);
		}
	Printf ("Car %d exits road\n", c);
		...
		shm.eastcars --;
		if ( !shm.eastcars && shm.waitingwest ){
			Signal(shm.sems[NUMPOS+1]);
		}
		else if(shm.waitingeast && !shm.waitingwest){
			Signal(shm.sems[NUMPOS+2]);
		}
	Printf ("Car %d exits road\n", c);
	}		
